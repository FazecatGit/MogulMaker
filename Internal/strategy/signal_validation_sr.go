...
func (srv *SupportResistanceValidator) ValidateSignalWithSR(
	...
	// Calculate distances
	validation.DistanceToSupport = DistanceToSupport(currentPrice, support)
	validation.DistanceToResistance = DistanceToResistance(currentPrice, resistance)

	// Determine if price is at support or resistance
	atSupport := IsAtSupport(currentPrice, support)
	atResistance := IsAtResistance(currentPrice, resistance)
...
func GenerateSRQualityReport(bars []types.Bar) *SRQualityReport {
	...
	report.SupportLevel = FindSupport(bars)
	report.ResistanceLevel = FindResistance(bars)
	...
	if IsAtSupport(report.CurrentPrice, report.SupportLevel) {
		report.PricePosition = "AT_SUPPORT"
	} else if IsAtResistance(report.CurrentPrice, report.ResistanceLevel) {
